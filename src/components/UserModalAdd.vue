...
const allRole = ref(['ADMIN', 'STUDENT', 'LECTURER']);
        ...
        <form @submit.prevent="addUser" class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4 flex-col justify-center gap-1 text-sm">
          ...
          <div class="relative z-0 w-full mb-6 group">
            <label for="underline_select" class="sr-only">select</label>
             <label for=""
              class="mb-10 peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-placeholder-shown:scale-100
               peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Role</label>
            <select id="underline_select" required="" v-model="newUserData.role"
